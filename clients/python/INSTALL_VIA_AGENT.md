...
5. 我的 shell 里是否已经 export 了 `OPENAI_API_KEY`？（运行 `echo "OPENAI_API_KEY: ${OPENAI_API_KEY:+set}${OPENAI_API_KEY:-unset}"`，不要把 key 本身打出来）
...
另外看 daemon 启动的那一行 JSON 日志里的 `"routing_tier"` 字段——应该是 `"llm"`、`"embedding"` 或 `"keyword"`，告诉我实际走的是哪一档。
...
- daemon 日志里看到 `"routing_tier":"keyword"` 但你觉得应该是 `"llm"` → `OPENAI_API_KEY` 没有传给 daemon 进程；启动前确认 `echo $OPENAI_API_KEY` 不为空
