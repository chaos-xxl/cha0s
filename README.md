...
- 聚类(关键词共现 MVP,以及基于 OpenAI embedding 的语义聚类)
- 打包(把 inbox 碎片事务性地剪切成新话题空间)
- 生命周期(归档 / 复活 / 合并 / 重命名)
- 纠正学习(用户手动修正会影响后续路由)
- OpenAI embedding 适配包 [`@doctorchaos-ai/openai`](https://www.npmjs.com/package/@doctorchaos-ai/openai)(一行代码从关键词匹配切换到语义路由)
- 7 个 LLM 路由适配包,覆盖国产与海外主流厂商:
  - `@doctorchaos-ai/openai` — OpenAI / Azure / OpenRouter / 任何 OpenAI 兼容端点
  - `@doctorchaos-ai/anthropic` — Claude 3.5 全系列
  - `@doctorchaos-ai/deepseek` — deepseek-chat / deepseek-reasoner
  - `@doctorchaos-ai/kimi` — Moonshot 全系列
  - `@doctorchaos-ai/zhipu` — GLM-4 全系列
  - `@doctorchaos-ai/qwen` — 通义千问全系列(DashScope 兼容模式)
  - `@doctorchaos-ai/minimax` — MiniMax Text / abab 全系列
  - `@doctorchaos-ai/doubao` — 豆包(Volcengine Ark 端点)
...
- Clustering (keyword co-occurrence MVP, plus OpenAI-embedding–backed semantic clustering)
...
- OpenAI embedding adapter [`@doctorchaos-ai/openai`](https://www.npmjs.com/package/@doctorchaos-ai/openai) (swap keyword matching for semantic routing in one line)
- Seven LLM routing adapters covering Chinese and international providers:
  - `@doctorchaos-ai/openai` — OpenAI / Azure / OpenRouter / any OpenAI-compatible endpoint
  - `@doctorchaos-ai/anthropic` — full Claude 3.5 line-up
  - `@doctorchaos-ai/deepseek` — deepseek-chat / deepseek-reasoner
  - `@doctorchaos-ai/kimi` — Moonshot family
  - `@doctorchaos-ai/zhipu` — GLM-4 family
  - `@doctorchaos-ai/qwen` — Alibaba Qwen (DashScope compatible mode)
  - `@doctorchaos-ai/minimax` — MiniMax Text / abab family
  - `@doctorchaos-ai/doubao` — Doubao (Volcengine Ark endpoints)
