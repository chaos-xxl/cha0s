...
[中文](#为什么做这件事) · [English](#why-this-exists)
...
flowchart TD
    ...
    B -->|什么都没传| E{process.env 有<br/>OPENAI_API_KEY?}
    E -->|有| D
    E -->|没有| F[第一档<br/>关键词匹配]

    C -.->|准确率 95-99%<br/>延迟 300-800ms<br/>每条 ~$0.001| G[路由决定]
    D -.->|准确率 90-95%<br/>延迟 100-300ms<br/>每条 ~$0.0001| G
    F -.->|准确率 60-75%<br/>延迟 &lt;10ms<br/>免费| G
```

### 第一档:关键词匹配(零配置,开箱即用)

**触发条件**:你什么都不传,环境变量也没有 `OPENAI_API_KEY`。
...
### 第二档:Embedding 路由(自动嗅探 / 显式传入)

**触发条件**(二选一):

- 你在 `process.env.OPENAI_API_KEY` 里存了一个 OpenAI key,Doctor Chaos 会自动嗅到并升级(也支持 `OPENAI_BASE_URL` 指向 Azure / OpenRouter / LiteLLM 等兼容端点)
- 你显式传 `embed: openaiEmbed({ apiKey: ... })`
...
**触发条件**:你传 `llm: someProvider({ apiKey: ... })`,传什么厂商都行。
...
如果你 key 不在 env 里,或想精确控制 model / baseUrl:
...
### 场景 B:你用 Claude / 国产大模型

```bash
npm install @doctorchaos-ai/core @doctorchaos-ai/deepseek
# 把 deepseek 换成你用的厂商:anthropic / kimi / zhipu / qwen / minimax / doubao
```
...
换厂商只需要换一行 import。完整适配包列表见[下文](#官方适配包)。
...
零配置就能跑,只是准确率只有 60-75%。**不推荐生产用**,但做 POC、测试、demo 完全够。
...
前面讲了"怎么用"。这里讲"它内部在想什么"——如果你要把 Doctor Chaos 集成进自己的产品里,这一节帮你理解它的状态机。
...
- 生命周期(归档 / 复活 / 合并 / 重命名)
- 纠正学习(用户手动修正会影响后续路由)
- 完整 TypeScript 类型定义、249 个单元测试、CI 覆盖 Node 18/20/22
...
Doctor Chaos runs out of the box, but accuracy tracks the "eyes" you give it. Top to bottom:
...
flowchart TD
    ...
    B -->|nothing| E{OPENAI_API_KEY<br/>in process.env?}
    E -->|yes| D
    E -->|no| F[Tier 1<br/>Keyword matching]

    C -.->|95-99% accuracy<br/>300-800ms<br/>~$0.001/msg| G[Routing decision]
    D -.->|90-95% accuracy<br/>100-300ms<br/>~$0.0001/msg| G
    F -.->|60-75% accuracy<br/>&lt;10ms<br/>free| G
```

### Tier 1: Keyword matching (zero config)

**Triggered when**: You pass nothing, and no `OPENAI_API_KEY` is in env.

**How it works**: Simple string matching between the incoming message and each space's keywords. The space with the highest hit ratio wins.

**Characteristics**:

- ✅ Zero dependencies, zero config, fully offline
- ✅ Latency <10ms, essentially imperceptible
- ✅ Free — run it as much as you want
...
### Tier 2: Embedding routing (auto-detect / explicit)

**Triggered when** (either one):

- `process.env.OPENAI_API_KEY` is set. Doctor Chaos auto-detects it and upgrades. Also respects `OPENAI_BASE_URL` for Azure / OpenRouter / LiteLLM / other OpenAI-compatible endpoints.
- You explicitly pass `embed: openaiEmbed({ apiKey: ... })`.

**How it works**: Embed both the message and each space's keyword centroid. Cosine similarity wins.

**Characteristics**:

- ✅ 90-95% accuracy; synonyms and semantic neighbors match naturally
- ✅ 100-300ms latency
- ✅ ~$0.0001 per message — effectively rounding-error cost
- ⚠️ Only works with providers that ship an embedding endpoint (OpenAI and OpenAI-compatible)

**Use when**: The default for most production users.

### Tier 3: LLM routing (works with any chat model)

**Triggered when**: You pass `llm: someProvider({ apiKey: ... })` — any provider works.

**How it works**: Doctor Chaos builds a prompt containing the incoming message plus every active space's name and keywords, then asks the chat model which space the message belongs to (or "new space" or "inbox").
...
- ✅ The only reliable path for providers without embedding endpoints (Anthropic, DeepSeek, etc.)
- ⚠️ 300-800ms latency (one model call per routed message)
- ⚠️ ~$0.001 per message — 10× the embedding cost

**Use when**: You need the highest accuracy, or your LLM provider doesn't offer embeddings.

---

## Quick start

### Scenario A: You use OpenAI (or an OpenAI-compatible endpoint)

```bash
npm install @doctorchaos-ai/core
```

If `OPENAI_API_KEY` is in your environment, Doctor Chaos auto-upgrades to embedding routing — nothing else to configure:
...
If your key isn't in env, or you want fine control over model / baseUrl:
...
### Scenario B: You use Claude or a Chinese LLM

```bash
npm install @doctorchaos-ai/core @doctorchaos-ai/deepseek
# Replace `deepseek` with your provider:
# anthropic / kimi / zhipu / qwen / minimax / doubao
```
...
Switching providers is a one-line import change. Full adapter list [below](#official-adapters).

### Scenario C: You don't want to configure any key
...
Zero config. Accuracy caps at 60-75%. **Not recommended for production**, but fine for POCs, tests, and demos.
...
// 1. Send a message — Doctor Chaos picks a destination.
const result = await clinic.send({
  role: 'user',
  content: 'Book me a flight to Kyoto next week.',
});
...
// 2. Read state — e.g. to render a sidebar.
const spaces = clinic.spaces({ status: 'active' });
const inbox = clinic.inbox();

// 3. User correction — the router learns from it.
await clinic.moveMessage(messageId, targetSpaceId);

// 4. Periodic maintenance — safe to run any time.
await clinic.checkPackaging();
await clinic.checkLifecycle();

// 5. Persistence — snapshot and rehydrate.
const snapshot = clinic.snapshot();
// On restart: new Clinic({ initialSpaces: snapshot.spaces, initialInbox: snapshot.inbox, ... })
```
...
Install the one you need. Mix and match freely.
...
| [`@doctorchaos-ai/minimax`](https://www.npmjs.com/package/@doctorchaos-ai/minimax)     | MiniMax                                                                | `MiniMax-Text-01`                        |
| [`@doctorchaos-ai/doubao`](https://www.npmjs.com/package/@doctorchaos-ai/doubao)       | Doubao (Volcengine Ark)                                                | Ark endpoint id (you create it)          |

Not on the list? Use core's `openAiCompatibleLLM` factory to wire up any OpenAI-compatible endpoint in one line — Groq, Together, self-hosted proxies, anything.

---

## The hospital metaphor: what Doctor Chaos looks like inside

The previous sections covered "how to use it". This one covers "what it's thinking about" — useful if you're integrating Doctor Chaos into a product and want to understand its state machine.
...
- Routing — each new message gets a destination
- Packaging — when enough fragments cluster around a theme in general practice, they're cut out and packaged into a new specialty
- Lifecycle — specialties go dormant when inactive; users talking about related things automatically revives them

The library does triage. Your agent keeps doing what it already does — it's just that now, the messages it processes have already been sorted.
...
Doctor Chaos is in **alpha**. "Alpha" isn't a disclaimer here, it's a commitment — the public API is feature-complete for v0.1.0 scope but may shift between minor versions based on real-world integration feedback.

**What works today**

- Three-tier routing: keyword / embedding / LLM
- Eight official adapters covering major international and Chinese LLM providers
...
- 🔗 [npm organization @doctorchaos-ai](https://www.npmjs.com/org/doctorchaos-ai)
...
The project was previously known as **cha0s** and is still visible on npm as `@cha0s-ai/core` (deprecated — please use `@doctorchaos-ai/core`). The renaming took a while, mostly because the hospital metaphor hadn't clicked yet — only when front desk, general practice, and specialty crystallized as three distinct layers did it become clear the project was really about triage.
